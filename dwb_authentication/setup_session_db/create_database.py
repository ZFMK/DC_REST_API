...
class SessionDBSetup():
	...
	def set_connectors(self):
		...
		if len(self.connectors) > 0:
			...
			(`accronym`, `server`, `port`, `database`, `driver`, `trust_certificate`, `trusted_connection`, `encrypt`, `collation`)
			VALUES {0}
		...
		for section in sections:
			if section[:6]=='dc_db_' and section!='dc_db_test' and len(section[6:]) > 0:
				...
				if server is not None and port is not None and database is not None:
					self.connectors.append([accronym, server, port, database, driver, trust_certificate, trusted_connection, encrypt, collation])
